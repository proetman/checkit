...
def  cleanup_box(img, min_x, min_y, max_x, max_y ):
    box = img[min_y:max_y, min_x:max_x ]
    height, width,_ = img.shape
    print('box {}:{}  {}:{}. h w = {},{}'.format(min_x, max_x, min_y, max_y, height, width))

    processed_img = cv2.Canny(box, threshold1=50, threshold2=500)
    cv2.imshow('box', processed_img)
...
#    y = []
    ...
    while cap.isOpened():
        ...
        if curr_min2 is not None:
            ...
            ret, mask = cv2.threshold(motion_detection_frame, 5, 255, cv2.THRESH_BINARY_INV, 5)
            img = cv2.GaussianBlur(motion_detection_frame, (5,5), 0)
            binImg = cv2.adaptiveThreshold(img, 255,  cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY,59, 1)
#                                           (img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 115, 1)
            ...
            processed_img = cv2.Canny(binImg, threshold1=200, threshold2=300)
...
#                    cv2.rectangle(frame, (x,y), (x+w,y+h), (255, 0, 0), 2)
            ...
            cv2.imshow('im2', binImg)


            if (sum_pix > MOVEMENT_THRESHOLD):
                have_car = True
                cleanup_box(orig_frame, min_x, min_y, max_x, max_y)
